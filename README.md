...
**M3 — Distributed Evaluation** is under way. The outbox relay delivers published-version events (#18), and the Caffeine L1 cache with invalidation and a staleness budget is in place (#19, first half). Redis L2 and reconciliation (#19, second half), the Java SDK and OpenFeature provider (#20), and reproducible benchmarks (#21) remain open.
...
O **M3 — Avaliação Distribuída** está em andamento. O relay do outbox entrega eventos de versão publicada (#18), e o cache L1 com Caffeine, invalidação e orçamento de defasagem está no lugar (#19, primeira metade). O Redis L2 e a reconciliação (#19, segunda metade), o SDK Java e o provider OpenFeature (#20) e os benchmarks reprodutíveis (#21) seguem abertos.
